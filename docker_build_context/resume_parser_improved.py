...
class ResumeParser:
            ...
            if date_match:
                ...
                if ' | ' in line_stripped:
                    ...
                    if date_seg_idx is not None and len(parts) >= 5 and date_seg_idx == 2:
                        current_job.title = parts[4].strip() if len(parts) > 4 else current_job.title
    ...
    def parse_resume(self, file_path: str) -> ParsedResume:
        ...
        if professional_summary_section and len(professional_summary_section.strip()) > 20:
            ...
            # Look for section header and extract content after it, stopping at next section
            summary_patterns = [
        ...
        if not experience_section or len(experience_section.strip()) < 50:
            ...
            # If section had few jobs but full text has more date ranges, section may have been truncated
            # (e.g. "Technologies" misclassified as skills). Re-extract from full text and keep if better.
